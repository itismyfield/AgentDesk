...
pub async fn send_handler(
    ...
    let sqlite = state.pg_pool_ref().is_none().then_some(state.sqlite_db());
    let (status_str, response_body) =
        health::handle_send_with_backends(registry, sqlite, state.pg_pool_ref(), &body_str).await;
...
pub async fn send_to_agent_handler(
    ...
    let body_str = String::from_utf8_lossy(&body);
    let sqlite = state.pg_pool_ref().is_none().then_some(state.sqlite_db());
    let (status_str, response_body) = health::handle_send_to_agent_with_backends(
        registry,
        sqlite,
        state.pg_pool_ref(),
        &body_str,
