use anyhow::Result;
use sqlite_test::{Connection, OptionalExtension, params_from_iter};
...
pub fn migrate(conn: &Connection) -> Result<()> {
    ...
    let version: i64 = conn
        .query_row(
            "SELECT COALESCE(CAST((SELECT value FROM kv_meta WHERE key = 'schema_version') AS INTEGER), 0)",
            [],
            |r| r.get(0),
        )
        .unwrap_or(0);
    ...
    // #667: Cancel duplicate active reviews on non-done cards before adding DB uniqueness.
    // Keeps only the most recent (highest rowid) active review per card.
    let review_dup_cancelled: usize = conn.execute(
        "UPDATE task_dispatches
        ...
        [r#"{"reason":"startup_reconcile_duplicate_review"}"#],
    )?;
    if review_dup_cancelled > 0 {
        tracing::info!(
            "Cancelled {review_dup_cancelled} duplicate active reviews at startup (#667)"
        );
    }

    // #667: Re-point latest_dispatch_id to the surviving active review dispatch.
    // #116 review-decision reconciliation still runs immediately after this and keeps
    // review-decision precedence if a legacy card somehow has both active types.
    let review_repointed: usize = conn
        .execute(
            "UPDATE kanban_cards SET latest_dispatch_id = ( \
             ...
             WHERE id IN ( \
                 ...
                 WHERE td2.dispatch_type = 'review' \
        ...
        .unwrap_or(0);
    if review_repointed > 0 {
        tracing::info!(
            "Re-pointed latest_dispatch_id on {review_repointed} card(s) to active review dispatches (#667)"
        );
    }
    ...
    // #667: Partial unique index — at most 1 active review per card at DB level.
    let _ = conn.execute_batch(
...
fn auto_queue_has_column(conn: &Connection, table: &str, column: &str) -> bool {
    conn.query_row(
        ...
        sqlite_test::params![table, column],
        |row| row.get(0),
    )
    .unwrap_or(false)
}
...
fn rebuild_auto_queue_phase_gates_table(conn: &Connection) -> Result<()> {
    ...
    let mut rows = conn
        ...
        .collect::<sqlite_test::Result<Vec<_>>>()?;
...
fn backfill_auto_queue_dispatch_history(conn: &Connection) -> Result<()> {
    ...
    for dispatch in dispatches {
        ...
        conn.execute(
            ...
            sqlite_test::params![entry_id, dispatch_id, created_at],
        )?;
    }
...
fn backfill_session_agent_ids(conn: &Connection) -> Result<()> {
    ...
    for (session_key, thread_channel_id, dispatch_id) in sessions {
        ...
        conn.execute(
            ...
            sqlite_test::params![session_key, agent_id],
        )?;
    }
        ...
        for (card_id, legacy_status, existing_blocked_reason) in legacy_cards {
            let prior_status = conn
                .query_row(
                    ...
                    sqlite_test::params![card_id, legacy_status],
                ...
                conn.execute(
                    "UPDATE kanban_cards
                     SET status = ?1,
                         ...
                         updated_at = datetime('now')
                     WHERE id = ?3",
                    sqlite_test::params![target_status, blocked_reason, card_id],
                )?;
                conn.execute(
    ...
    for (id, session_key, dispatch_id) in transcripts {
        ...
        conn.execute(
            "UPDATE session_transcripts
             ...
             WHERE id = ?1
               AND NULLIF(TRIM(agent_id), '') IS NULL",
            sqlite_test::params![id, agent_id],
        )?;
    }
    ...
    fn backfill_auto_queue_phase_gates_chunks_large_fk_lookups() {
        ...
        for index in 0..total {
            ...
            conn.execute(
                ...
                sqlite_test::params![card_id, format!("Phase Gate Card {index}")],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO auto_queue_runs (id, repo, agent_id, status)
                 VALUES (?1, 'test/repo', 'agent-1', 'paused')",
                sqlite_test::params![run_id],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO task_dispatches (
                ...
                sqlite_test::params![dispatch_id, card_id, format!("Phase Gate Dispatch {index}")],
            )
            .unwrap();
            conn.execute(
