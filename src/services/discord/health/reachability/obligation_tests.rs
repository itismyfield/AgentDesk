...
//! gate (#5071 T4-B2 = 4987 §-1.5 blocker B1′).
//!
...
fn a_partial_line_holds_the_cursor_at_its_first_byte() {
    ...
    assert_eq!(
        scan.next_offset, 16,
        "the cursor must stop AT the partial line so the next tick frames it whole"
    );
    assert_eq!(scan.obligations().count(), 0);
}

/// A chunk boundary inside a UTF-8 sequence must not reach a decoder at all.
/// The bytes are re-read whole next tick, so half a character can never be
