...
//!   [`ObligationScan::next_offset`] past it, so the next tick re-reads those
...
//!   to resume a cursor whose `(dev, ino)` moved, and the observation task
//!   re-bootstraps. This file only guarantees that a record can never be
//!   attributed to a file it was not read from.
//! * **Oversized line** — a single line longer than `oversized_line_limit`
//!   would otherwise pin `next_offset` forever, because no tick could ever see
//!   its terminator. It is emitted as [`ObligationReason::OversizedLine`] and
//!   the cursor advances. It is NOT an obligation, and the observation task
//!   turns it into `Unknown{ReadTruncated}` — the tick did not see a whole
//!   record, and 4987 §-1.4 makes "did not see" non-GREEN rather than absent.
//!
...
///
/// No production caller, by design: the canonical ENCODING exists to be
/// compared against the Python half, and its consumers are this file's fixture
/// test and `scripts/check_reachability_canonical_equivalence.py`. The
/// observation task consumes [`scan_canonical`], not its serialization.
#[allow(dead_code)]
...
pub(in crate::services::discord) enum ObligationReason {
    ...
    AssistantText,
    /// The chunk ended without a terminator. Re-read next tick, never an
...
impl ObligationReason {
    ...
    ///
    /// Consumed by the encoding, hence by the equivalence corpus, not by the
    /// observation task — see [`CANONICAL_SCHEMA_HEADER`].
    #[allow(dead_code)]
    ...
    /// Whether this line means the tick did not see a whole record, and the
    /// observation must therefore report `Unknown{ReadTruncated}` rather than
    /// treating the gap as "nothing was there".
...
pub(in crate::services::discord) struct ObligationScan {
    ...
    /// points at that line's first byte so the next tick reads it whole.
...
/// of 4987 §-1.3 that the observation task resolved before reading.
pub(in crate::services::discord) fn scan_canonical(
    ...
    ObligationScan {
        records,
        // Deliberately NOT past the partial line: next tick re-reads it whole.
...
///
/// No production caller — see [`CANONICAL_SCHEMA_HEADER`] for why the encoding
/// is corpus-facing rather than runtime-facing.
#[allow(dead_code)]
