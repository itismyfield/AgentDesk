...
use mailbox::WATCHER_STATE_DESYNC_STALE_MS;
#[allow(unused_imports)]
pub(crate) use mailbox::stop_provider_channel_runtime_with_policy;
#[allow(unused_imports)]
pub use mailbox::{
    HardStopRuntimeResult, PendingQueueSnapshot, PostCancelDrainOutcome, RuntimeTurnStopResult,
    active_request_owner_for_channel, clear_provider_channel_runtime,
    force_kill_provider_channel_runtime, hard_stop_runtime_turn, resolve_tmux_session_for_cancel,
    schedule_pending_queue_drain_after_cancel, snapshot_pending_queue_state,
    stop_provider_channel_runtime, stop_runtime_turn_preserving_watcher,
};

use mailbox::{MailboxHealthSnapshot, summarize_provider_mailboxes};

#[cfg(all(test, feature = "legacy-sqlite-tests"))]
use super::relay_health::RelayActiveTurn;
...
impl HealthRegistry {
    ...
    async fn snapshot_watcher_state_filtered(
        ...
        mailbox::snapshot_watcher_state_filtered(self, channel_id, provider_filter).await
    }
...
async fn build_health_snapshot_with_options(
    ...
    for entry in providers.iter() {
        let session_count = entry
            ...
            .unwrap_or(0);
        let mailbox_summary = summarize_provider_mailboxes(entry, include_mailbox_details).await;
        let active_turns = mailbox_summary.active_turns;
        let provider_queue_depth = mailbox_summary.queue_depth;
        ...
        let recovering_channels = mailbox_summary.recovering_channels;
        ...
        mailbox_entries.extend(mailbox_summary.entries);
