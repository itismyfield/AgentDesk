...
fn relay_ownership_uses_session_bound_inflight_shape() {
    ...
    let mut external_session_bound = inflight_for(tmux, RelayOwnerKind::SessionBoundRelay, false);
...
fn idle_relay_delivers_suffix_when_init_is_in_committed_prefix() {
    ...
    let action = idle_relay_range_action(full_bytes, start, end, committed, false, false, false);
    assert_eq!(
        action,
...
fn cutover_short_replace_production_path_advance_is_fresh_identity_gated() {
    ...
    let sink = SessionBoundDiscordRelaySink::new(Arc::new(HealthRegistry::new()));
    let matching =
        inflight_with_identity_offset(channel.get(), session, 0, "2026-06-04T00:00:00Z", Some(0));
    ...
    super::super::inflight::save_inflight_state(&mismatched).expect("persist mismatched inflight");
...
async fn sink_identity_gate_blocks_advance_on_wrong_turn() {
    ...
    let replaced_inflight =
        inflight_with_identity_offset(channel.get(), session, 999, "2026-06-04T00:00:00Z", Some(0));
    sink.advance_offset_for_confirmed_delegated_terminal(
        &shared,
        &ProviderKind::Claude,
        channel.get(),
...
async fn sink_identity_gate_blocks_same_second_turn_by_turn_start_offset() {
    ...
    let new_turn_frame = delivery_with_fence_offset(session, Some(512), 0, same_second, Some(4096));
    sink.advance_offset_for_confirmed_delegated_terminal(
        &shared,
        &ProviderKind::Claude,
        channel.get(),
...
async fn sink_strict_gate_blocks_advance_when_frame_offset_is_none() {
    ...
    let none_offset_frame = delivery_with_fence_offset(session, Some(256), 0, same_second, None);
...
fn session_bound_lease(channel_id: u64, session: &str, turn_seq: u64) -> ExternalInputRelayLease {
...
mod inflight_sink_marker {
    ...
    async fn acquire_sets_leased_sink_marker() {
        ...
        let guard = SinkDeliveryLeaseGuard::acquire(&cell, turn, START, END).expect("acquire wins");
        match cell.read() {
