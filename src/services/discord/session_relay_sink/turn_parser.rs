...
pub(in crate::services::discord) struct SessionRelayParser {
    ...
    /// #5948 (I17): the exclusive end of the source byte range already folded
...
impl SessionRelayParser {
    pub(super) fn ingest_verified_native_terminal(
        ...
        // #5948 (I17): the payload below is SYNTHESISED here, not read from the
    ...
    /// #5948 (I17): append the part of `frame.payload` this parser has not
    ...
    /// #5948 (I17): a dropped resend must never be silent — #5941 is the
    ...
    /// #5948 (I17): the resend watermark, for tests that pin sink parser state
    /// across the two watcher rewind paths.
    #[cfg(test)]
    pub(super) fn turn_source_end(&self) -> Option<u64> {
        self.turn_source_end
    }

    /// #5948 (I17): bytes still awaiting a turn terminator, for the same tests.
    #[cfg(test)]
