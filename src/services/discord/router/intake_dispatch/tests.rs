...
#[rustfmt::skip]
async fn insert_raw_delivery_journal_row(pool: &sqlx::PgPool, obligation: uuid::Uuid, attempt: Option<uuid::Uuid>, kind: &str, seq: i16, payload: serde_json::Value, receipt: [Option<&str>; 3]) {
    sqlx::query(
        ...
        .execute(pool).await.expect("seed raw journal row");
}

#[rustfmt::skip]
#[tokio::test(flavor = "current_thread")]
async fn obligation_window_facade_restores_sequence_and_receipt_pg() {
    let db=TestPostgresDb::create().await; let pool=db.connect_and_migrate().await;
    let obligation=uuid::Uuid::new_v4(); let attempt=uuid::Uuid::new_v4();
    insert_raw_delivery_journal_row(&pool,obligation,Some(attempt),"C",3,serde_json::json!({"frontier_start":10,"frontier_end":20}),[None,None,None]).await;
    insert_raw_delivery_journal_row(&pool,obligation,None,"O",0,serde_json::json!({"intake_outbox_id":42}),[None,None,None]).await;
    insert_raw_delivery_journal_row(&pool,obligation,Some(attempt),"T",2,serde_json::json!({"requested_channel_id":"10","returned_channel_id":"10","message_id":"20"}),[Some("10"),Some("10"),Some("20")]).await;
    insert_raw_delivery_journal_row(&pool,obligation,Some(attempt),"A",1,serde_json::json!({"frontier_start":10,"frontier_end":20}),[None,None,None]).await;
    let judgment=crate::services::discord::session_relay_sink::journal::judge_obligation_window(&pool,obligation).await.expect("read raw window");
...
async fn obligation_window_facade_rejects_unknown_kind_pg() {
    ...
    insert_raw_delivery_journal_row(&pool,obligation,None,"future",9,serde_json::json!({}),[None,None,None]).await;
    let judgment=crate::services::discord::session_relay_sink::journal::judge_obligation_window(&pool,obligation).await.expect("read unknown-kind window");
