...
async fn render_current_head_card(
    ...
    shared
        ...
        .await;
    // Every caller already owns the channel's queued-placeholder persistence
    // lock and has keyed this card to the current head. Re-locking it here
    // would deadlock after a successful Discord PATCH because Tokio mutexes are
    // not reentrant.
    Ok(true)
}
...
async fn native_tui_steering_context(
    ...
    #[cfg(unix)]
    let tmux_available = crate::services::claude::is_tmux_available();
    #[cfg(not(unix))]
