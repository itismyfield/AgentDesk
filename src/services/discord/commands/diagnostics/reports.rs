//! Diagnostic report builders extracted from `diagnostics.rs` (#3479).
//!
//! Verbatim move of the runtime/channel/inflight/queue report string builders
//! plus the snapshot-normalization and runtime-labeling helpers they consume.
//! Behavior-preserving: bodies are byte-identical to the originals; only the
//! module path depth (`super::super::` -> `super::super::super::`) and the
//! re-export visibility were adjusted for the new child-module location.
...
use super::super::super::formatting::truncate_str;
use super::super::super::inflight::{InflightTurnState, load_inflight_states};
use super::super::super::runtime_store;
use super::super::super::{
    Intervention, PendingQueueItem, SharedData, mailbox_queue_snapshots, mailbox_snapshot,
};
...
async fn fetch_raw_provider_session_id(
    ...
    super::super::super::internal_api::get_provider_session_id(session_key, Some(provider.as_str()))
        .await
        .ok()
        .and_then(|json| {
...
pub(in crate::services::discord) async fn build_health_report(
    ...
    let session_key = tmux_session_name.as_ref().map(|session_name| {
        super::super::super::adk_session::build_namespaced_session_key(
            &shared.token_hash,
            provider,
            session_name,
        )
