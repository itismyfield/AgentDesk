...
impl SessionBoundDiscordRelaySink {
    ...
    async fn deliver_short_replace_via_controller<G: super::gateway::TurnGateway + ?Sized>(
        ...
        match outcome {
            ...
            // #4046 S1r-1 P2: a `FreshDelivered` outcome is an UNREACHABLE cross-verb
            // result here — `SendFresh` is not this stage's short-replace plan. It is a
            // CONFIRMED POST (see `DeliveryOutcome::FreshDelivered` docs), so it must
            // NEVER be surfaced as retriable `Transient`: a blind retry would duplicate
            // the already-landed POST (dedup-less when `persistence_recorded == false`).
            // The pure classifier maps it to the NON-retriable `Permanent` arm and
            // claims no delivery — mirroring the control site
            // `tmux_watcher/terminal_send.rs`, where `FreshDelivered { .. }` maps to the
            // conservative non-retry `WatcherShortReplaceResult::Skipped`. Every other
            // non-delivery here (ambiguous `PartialContinuationFailure` / transport Err,
            // lost-acquire `Transient`, empty-body `Skipped`) is genuinely uncommitted
            // (I2 — offset NOT advanced) → retriable `Transient`. Routed through the
            // pure free fn so this retry classification is unit-testable; reverting the
            // `FreshDelivered` arm there to `Transient` re-arms the duplicate-POST
            // booby-trap this fix closed (see
            // `fresh_delivered_short_replace_outcome_is_not_retriable`).
            ...
            | toc::DeliveryOutcome::Skipped) => {
                Err(short_replace_controller_non_delivery_error(&non_delivery))
            }
        }
    }
...
/// #4046 S1r-1 P2: classify a session-bound short-replace controller outcome that did
/// NOT confirm a placeholder edit into a sink error, keeping the retry classification
/// pure and unit-testable (the inline `Delivered`/`NotDelivered` arm in
/// `deliver_short_replace_via_controller` carries metrics/tracing/observability side
/// effects and cannot be exercised in isolation).
///
...
/// `Transient`.
fn short_replace_controller_non_delivery_error(outcome: &toc::DeliveryOutcome) -> RelaySinkError {
    match outcome {
...
mod tests {
    ...
    /// arm of `short_replace_controller_non_delivery_error` back to `Transient` FAILS
    /// this assert.
    #[test]
    fn fresh_delivered_short_replace_outcome_is_not_retriable() {
        ...
        let err = short_replace_controller_non_delivery_error(&fresh);
        assert!(
        ...
        for uncommitted in [
            ...
            let err = short_replace_controller_non_delivery_error(&uncommitted);
            assert!(
