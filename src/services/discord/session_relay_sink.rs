...
mod tests {
    ...
    fn session_sink_short_replace_raw_body_fingerprint_refuses_watcher_rerelay_4081() {
        ...
        let module_src = include_str!("session_relay_sink/delivery.rs");
        let format_src = include_str!("session_relay_sink/relay_format.rs");
        assert!(
            module_src.contains(
                "relay_format::session_bound_relay_bodies(&shared, &provider, &delivery)"
            ),
            "session sink must delegate raw/display body derivation to the format boundary"
        );
        assert!(
            format_src.contains("let raw_response_text = delivery.response_text.clone();"),
            "format boundary must preserve raw response bytes before Discord formatting"
        );
        assert!(
            format_src.contains("format_for_discord_with_provider(&raw_response_text"),
            "session sink must format from the preserved raw body, not derive raw from formatted text"
        );
        assert!(
            format_src.contains("format_for_discord_with_status_panel(&raw_response_text"),
