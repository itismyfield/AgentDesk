...
impl TurnGateway for TuiDirectBridgeGateway {
    ...
    fn send_long_message_with_rollback<'a>(
        ...
        Box::pin(async move {
            super::formatting::send_long_message_raw_with_rollback(
                &self.http,
                channel_id,
...
async fn relay_tui_idle_response_through_bridge(
    ...
    match tokio::time::timeout(Duration::from_secs(180), completion_rx).await {
        Ok(_) => {
            if let Some(error) = tui_direct_bridge_uncommitted_delivery_error(
                &provider,
                channel_id,
                user_msg_id,
                current_msg_id,
...
mod tests {
    ...
    fn tui_direct_bridge_completion_rejects_uncommitted_matching_inflight() {
        ...
        super::super::inflight::save_inflight_state(&fallback_delivered)
        ...
        super::super::inflight::save_inflight_state(&committed).expect("save committed inflight");
        assert!(
            tui_direct_bridge_uncommitted_delivery_error(
                &provider,
                channel_id,
