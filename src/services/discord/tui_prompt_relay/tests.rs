...
fn synthetic_bridge_handoff_fixture(
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async {
            ...
            lease.session_key = Some(super::super::adk_session::build_namespaced_session_key(
                &shared.token_hash,
                &provider,
                tmux,
            ));
            ...
            let claim = async {
                ...
                let result = super::synthetic_start::claim_tui_direct_synthetic_turn(
                    &shared,
                    &provider,
                    channel,
                    tmux,
            ...
            if foreign_actor || wrong_source {
                claim.await;
                let original = super::super::mailbox_snapshot(&shared, channel)
                    .await
                    .cancel_token
                    .unwrap();
                if foreign_actor {
                ...
                assert!(
                    super::synthetic_start::bridge_handoff::capture(
                        &shared,
                        &provider,
                        channel,
                        tmux,
            ...
            let capture = super::synthetic_start::bridge_handoff::capture(
            ...
            let owner = super::super::mailbox_snapshot(&shared, channel).await;
            assert!(Arc::ptr_eq(
            ...
            let active = super::super::mailbox_snapshot(&shared, channel).await;
            assert!(Arc::ptr_eq(
            ...
            assert_eq!(
                tokio::time::timeout(Duration::from_secs(5), done_rx)
                    ...
                    .unwrap()
                    .unwrap(),
                super::super::turn_bridge::BridgeCompletionSignal::Finalized
            );
            assert!(
                super::super::mailbox_snapshot(&shared, channel)
                    .await
