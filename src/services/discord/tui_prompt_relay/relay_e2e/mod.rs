...
/// A `claude` stand-in that answers every turn with an immediate success on the bound session.
fn write_provider_stub(root: &std::path::Path) -> PathBuf {
