...
pub(super) async fn handle_empty_response_and_busy_requeue(
    ...
    let outcome = handle_empty_response_recovery(message, recovery_ctx, recovery_state).await;
    let (mut delivery_response, spoken_delivery_response, resume_retry_queued, silent_turn_handled) =
        match outcome {
