...
enum ProviderErrorPresentation {
    PromptTooLong(String),
    Failure(String),
}

fn provider_error_detail(message: &str, stderr: &str) -> String {
...
fn provider_error_presentation(message: &str, stderr: &str) -> ProviderErrorPresentation {
...
mod guidance_message_tests {
    ...
    fn prompt_too_long_guard_selects_actionable_folded_presentation() {
        ...
        assert!(
            response.starts_with("⚠️ 현재 대화가 provider의 컨텍스트 한도를 넘었어요.")
        );
