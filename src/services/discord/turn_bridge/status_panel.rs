...
pub(super) async fn complete_status_panel_v2<G: TurnGateway + ?Sized>(
    ...
    if !shared.ui.status_panel_v2_enabled {
        return StatusPanelCompletionResult::not_applicable(true);
    }
    ...
    match status_panel_completion_action(status_panel_msg_id, last_status_panel_text, &panel_text) {
        StatusPanelCompletionAction::AlreadyCommitted => {
            ...
            let binding_disposition = singleton::commit_completed_binding(
                shared,
                provider,
                channel_id,
                status_panel_msg_id,
            );
            if singleton::completion_commit_allows_pending_bind_purge(binding_disposition) {
...
pub(in crate::services::discord) async fn complete_status_panel_v2_with_http(
    ...
    complete_status_panel_v2_with_http_and_disposition(
        shared,
        http,
        channel_id,
        status_panel_msg_id,
...
pub(in crate::services::discord) struct StatusPanelCompletionResult {
    pub committed: bool,
...
impl StatusPanelCompletionResult {
    fn not_applicable(committed: bool) -> Self {
        Self {
            committed,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::services::discord) async fn complete_status_panel_v2_with_http_and_disposition(
    shared: &std::sync::Arc<SharedData>,
    http: &serenity::Http,
    channel_id: ChannelId,
    status_panel_msg_id: Option<MessageId>,
...
/// #3161: bridge completion must skip an edit when a newer turn owns the same
/// panel message, otherwise the old turn can overwrite the newer live panel.
///
