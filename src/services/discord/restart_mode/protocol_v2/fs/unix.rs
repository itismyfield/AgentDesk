...
mod high_risk_recovery {
    ...
    const SEALED_STAGE_OWNER_CHILD: &str = "AGENTDESK_5604_SEALED_STAGE_OWNER_CHILD";
    ...
    fn sealed_stage_link_records_identity_and_cleanup_stays_maintenance_only() {
        ...
        if std::env::var(SEALED_STAGE_OWNER_CHILD).as_deref() != Ok(ppid.as_str()) { let output = std::process::Command::new(std::env::current_exe().unwrap()).args([SEALED_STAGE_TEST, "--exact", "--nocapture", "--test-threads=1"]).env(SEALED_STAGE_OWNER_CHILD, std::process::id().to_string()).env_remove(MUTATION_LOCK_CHILD).output().unwrap(); assert_child(output); return }
