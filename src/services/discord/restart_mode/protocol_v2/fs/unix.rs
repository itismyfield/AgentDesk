...
mod high_risk_recovery {
    ...
    const EXPECTED_OPEN_FLAGS: libc::c_int =
        libc::O_RDONLY | libc::O_DIRECTORY | libc::O_CLOEXEC | libc::O_NOFOLLOW;
    #[cfg(target_os = "linux")]
    const SYMLINK_ERRNO: i32 = libc::ELOOP;
    #[cfg(target_os = "macos")]
    const SYMLINK_ERRNO: i32 = libc::ENOTDIR;
    fn assert_io(error: FsError, expected: (FsOperation, i32)) {
        let (operation, errno) = expected;
        assert_eq!(error.kind(), FsError::io(operation, errno).kind());
    }
    ...
    fn root_traversal_and_path_replacement_keep_the_pinned_inode() {
        ...
        let reject = |suffix: &str, ok: &[&[u8]], failed: &[u8], errno: i32| {
            ...
            let prior = successful_prefix(prefix, ok).1;
            assert_eq!(
        ...
        let trace = TRACE.with(|trace| std::mem::take(&mut *trace.borrow_mut()));
        let (final_fact, raw_fd, anchor_fd) = successful_prefix(&trace, &root_ok);
        ...
        let original = identity(&fs::metadata(root_path).unwrap());
        ...
        let replacement = identity(&fs::metadata(root_path).unwrap());
        ...
        assert_eq!(
            (root.lineage().root(), root.identity()),
            (original, original)
        );
        assert_eq!((final_fact.identity, pinned), (original, original));
        ...
        let trace = TRACE.with(|trace| std::mem::take(&mut *trace.borrow_mut()));
        let prior = successful_prefix(&trace[..2], &root_ok[..1]).1;
        ...
        let observed = (fstat.fd, fstat.result, fstat.errno);
        assert_eq!(observed, (open.result_fd, -1, Some(libc::EIO)));
        assert_eq!(fstat.fact, None);
