...
mod high_risk_recovery {
    ...
    fn root_traversal_and_path_replacement_keep_the_pinned_inode() {
        ...
        let successful_prefix = |trace: &[TraceResult], expected: &[&[u8]], keep_last: bool| {
            assert_eq!(trace.len(), expected.len() * 2);
            let mut pairs = trace.chunks_exact(2).zip(expected).peekable();
            let mut prior = libc::AT_FDCWD;
            let mut final_fact = None;
            while let Some((pair, component)) = pairs.next() {
                ...
                if !keep_last || pairs.peek().is_some() {
        ...
        let reject = |suffix: &str, ok: &[&[u8]], failed: &[u8], errno: i32| {
            ...
            let prior = successful_prefix(prefix, ok, false).1;
            assert_eq!(
        ...
        let trace = TRACE.with(|trace| std::mem::take(&mut *trace.borrow_mut()));
        let (final_fact, raw_fd) = successful_prefix(&trace, &root_ok, true);
        ...
        let trace = TRACE.with(|trace| std::mem::take(&mut *trace.borrow_mut()));
        let prior = successful_prefix(&trace[..2], &root_ok[..1], false).1;
