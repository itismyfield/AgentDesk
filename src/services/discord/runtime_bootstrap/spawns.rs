    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let sessions: Vec<_> = shared
                ...
                .collect();
            let bindings: Vec<_> = sessions
                .into_iter()
                .filter_map(|session| {
                    let channel = shared
                        .tmux_watchers
                        .owner_channel_for_tmux_session(&session)?;
                    let binding = shared.tmux_watchers.channel_binding(&channel)?;
                    (binding.tmux_session_name == session).then_some(binding)
                })
                .collect();
            ...
            let tick = tokio::task::spawn_blocking(move || {
                for binding in bindings {
                    let Some(transcript_path) = binding.output_path.as_deref() else {
                        tracing::warn!(
                    ...
                    let generation = outbound::delivery_record::current_generation_mtime_ns(
                        &binding.tmux_session_name,
                    );
                    let spawn_nonce = tmux::execution_identity::capture_spawn_nonce(
                        &binding.tmux_session_name,
                    );
