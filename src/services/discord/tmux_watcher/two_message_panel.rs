...
pub(in crate::services::discord) fn prepare_watcher_two_message_panel(
    two_message_panel_enabled: bool,
    shared: &SharedData,
    provider: &ProviderKind,
    channel_id: ChannelId,
    prior_binding: Option<
    ...
    Option<crate::services::discord::status_panel_transition::PreparedStatusPanelTransition>,
    String,
> {
    if !two_message_panel_enabled {
        return Ok(None);
    }
    crate::services::discord::status_panel_transition::prepare_candidate(
        provider,
        &shared.token_hash,
        channel_id.get(),
        prior_binding,
...
pub(in crate::services::discord) fn acknowledge_watcher_two_message_panel(
    two_message_panel_enabled: bool,
    shared: &SharedData,
    provider: &ProviderKind,
    channel_id: ChannelId,
    ...
    let prepared = prepared.ok_or_else(|| "watcher status panel intent missing".to_string())?;
    match crate::services::discord::status_panel_transition::acknowledge_candidate(
        provider,
        &shared.token_hash,
        channel_id.get(),
        prepared,
...
pub(in crate::services::discord) enum WatcherStatusPanelProtectionDisposition {
    Protected,
    Discarded,
    RetainedForRetry,
}

pub(in crate::services::discord) fn watcher_status_panel_protection_allows_bind(
    disposition: WatcherStatusPanelProtectionDisposition,
) -> bool {
    disposition == WatcherStatusPanelProtectionDisposition::Protected
}

fn watcher_status_panel_failed_protection_disposition(
    shared: &SharedData,
    provider: &ProviderKind,
    channel_id: ChannelId,
    panel_msg_id: serenity::MessageId,
    discard: &crate::services::discord::placeholder_cleanup::PlaceholderCleanupOutcome,
) -> WatcherStatusPanelProtectionDisposition {
    if watcher_status_panel_delete_needs_orphan_retry(discard) {
        return WatcherStatusPanelProtectionDisposition::RetainedForRetry;
    }
    let retirement = if discard.is_committed() {
    ...
    if crate::services::discord::status_panel_transition::finalize_retirement(
        provider,
        &shared.token_hash,
        channel_id.get(),
        panel_msg_id.get(),
...
pub(in crate::services::discord) async fn protect_or_discard_watcher_two_message_panel(
    ...
    let ((http, shared, provider), (channel_id, tmux_session_name, panel_msg_id, prepared)) =
        request;
    let Err(error) = acknowledge_watcher_two_message_panel(
        ...
        return WatcherStatusPanelProtectionDisposition::Protected;
    };
    tracing::warn!(
    ...
    .await;
    watcher_status_panel_failed_protection_disposition(
        shared.as_ref(),
        provider,
        channel_id,
        panel_msg_id,
...
pub(in crate::services::discord) async fn reanchor_watcher_two_message_status_panel_below_answer(
    ...
    let prior =
        crate::services::discord::status_panel_singleton_store::StatusPanelSingletonBinding {
            ...
            generation: *this_turn_status_panel_generation,
        };
    let prepared = match prepare_watcher_two_message_panel(
        ...
        channel_id,
        Some(prior),
        require_identity.clone(),
        panel_text,
    ) {
        Ok(Some(prepared)) => prepared,
        Ok(None) => return false,
        Err(error) => {
            tracing::warn!(channel_id = channel_id.get(), error = %error, "watcher could not persist re-anchor intent before send");
            return false;
        }
    };
    ...
    let new_panel = match crate::services::discord::http::send_channel_message_with_nonce(
        http,
        channel_id,
        &prepared.content,
        &prepared.nonce,
        ...
        Err(error) => {
                ...
                )
            {
                tracing::warn!(
                    ...
                    "watcher could not persist re-anchor send disposition"
                );
            }
    ...
    let protection = protect_or_discard_watcher_two_message_panel((
        ...
        (channel_id, tmux_session_name, new_panel.id, Some(&prepared)),
    ))
    .await;
