...
/// its managed tmux session is proven absent. The heal path retains the
/// identity and respawn guards in `heal_stale_busy_mailbox`; callers only need
/// to supply the inputs already available at the intake claim site.
pub(in crate::services::discord) async fn try_start_turn_with_stale_busy_heal(
    shared: &Arc<SharedData>,
    channel_id: serenity::ChannelId,
    ...
    let started = super::router::mailbox_try_start_turn_with_terminal_marker_cleanup(
        shared,
        channel_id,
        cancel_token.clone(),
        request_owner,
    ...
    if !started
        ...
        .await
    {
        return super::router::mailbox_try_start_turn_with_terminal_marker_cleanup(
            shared,
            channel_id,
            cancel_token,
            request_owner,
...
pub(in crate::services::discord) fn stale_busy_context<'a>(
...
pub(in crate::services::discord) async fn resolve_channel_tmux_names(
    shared: &Arc<SharedData>,
    provider: &ProviderKind,
    channel_id: serenity::ChannelId,
