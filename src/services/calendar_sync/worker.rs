...
// Retry only identity checks before dispatch; ambiguous writes require reconciliation.
const MAX_IDENTITY_ATTEMPTS: i32 = 5;
...
pub(crate) async fn tick(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    let identity = match client.calendar_identity().await {
        ...
        Err(error) => {
            ...
            ) && claim.attempts < MAX_IDENTITY_ATTEMPTS;
            return db::fail(
                pool,
                &claim,
