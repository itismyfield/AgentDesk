...
// Kakao Talk Calendar request limits; values are character counts or minutes.
...
const MAX_REMINDERS: usize = 2;
const MAX_REMINDER_MINUTES: i32 = 30 * 24 * 60;
const REMINDER_STEP_MINUTES: i32 = 5;
...
impl EventContent {
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if self.location.as_ref().is_some_and(|s| {
            s.name.trim().is_empty() || s.name.chars().count() > MAX_LOCATION_CHARS
        }) {
        ...
        if let Some(reminders) = &self.reminders {
            if reminders.is_empty()
                || reminders.len() > MAX_REMINDERS
                || reminders
                    .iter()
                    .any(|n| *n < 0 || *n > MAX_REMINDER_MINUTES || n % REMINDER_STEP_MINUTES != 0)
                || (reminders.len() == MAX_REMINDERS && reminders[0] == reminders[1])
            {
                return Err(
