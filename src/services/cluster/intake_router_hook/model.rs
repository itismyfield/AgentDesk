...
pub(crate) enum IntakeRouterDecision {
    ...
    /// A redelivery hit the attempt-1 unique constraint for this message.
    /// Caller MUST NOT run locally: an earlier path owns the execution.
    /// Unlike a DB-error fallback, this outcome skips the turn.
    SkippedDuplicate {
...
pub(crate) enum RanLocalReason {
    ...
    /// Disabled-mode observation: a configured preference is not enforced.
...
/// Per-channel input and execution policy passed by the intake gate.
#[derive(Clone, Debug)]
pub(crate) struct IntakeRouterContext<'a> {
    pub mode: IntakeRoutingMode,
    pub leader_instance_id: &'a str,
    /// Worker claims use the forwarding bot's provider, never `agents.provider`,
    /// which is shared between an agent's cc and cdx channels.
    pub provider: &'a str,
...
pub(super) fn build_payload_for_insert(
    ...
    InsertPendingPayload {
        ...
        dm_hint: ctx.dm_hint,
        // Emit "foreground"; the worker also accepts legacy "standard"
        // so already queued rows remain executable.
