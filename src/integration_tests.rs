...
mod tests {
    ...
    fn set_config_key(db: &db::Db, key: &str, value: serde_json::Value) {
        ...
        conn.execute(
            ...
            sqlite_params![key, value.to_string()],
        )
        .unwrap();
    }
    ...
    fn seed_dispatch(db: &db::Db, dispatch_id: &str, card_id: &str, dtype: &str, status: &str) {
        ...
        conn.execute(
            ...
            sqlite_params![dispatch_id, card_id, dtype, status],
        )
        .unwrap();
        conn.execute(
    ...
    fn phase_gate_state(db: &db::Db, run_id: &str, phase: i64) -> Option<serde_json::Value> {
        ...
        let mut stmt = conn
            ...
            .unwrap();
        let rows = stmt
            .query_map(sqlite_params![run_id, phase], |row| {
                Ok((
    ...
    fn seed_pr_tracking(
        ...
        conn.execute(
            ...
            sqlite_params![card_id, repo_id, worktree_path, branch, pr_number, head_sha, state],
        )
        .unwrap();
    }
    ...
    fn seed_stamped_create_pr_state(
        ...
        conn.execute(
            ...
            sqlite_params![dispatch_id, card_id, status, context],
        )
        .unwrap();
        conn.execute(
            "UPDATE kanban_cards SET latest_dispatch_id = ?1 WHERE id = ?2",
...

        {
            ...
            conn.execute(
                "INSERT INTO pipeline_stages (repo_id, stage_name, stage_order, trigger_after, provider, skip_condition) \
                 VALUES ('test/repo', 'test-stage', 100, 'review_pass', 'self', 'no_rs_changes')",
                [],
            )
            .unwrap();
        }
    ...
    /// `agentdesk.reviewAutomation.attemptCreatePr()` call would dispatch
    /// `create-pr` for noop work — resulting in an empty PR, wasted CI,
    /// and possible auto-merge of "no changes".
    #[tokio::test(flavor = "current_thread")]
    async fn scenario_701_noop_verification_pass_skips_pipeline_entry() {
        ...
        // Seed a pipeline stage — without the short-circuit this
        // would be entered by the review-pass handler.
        {
            ...
            conn.execute(
                "INSERT INTO pipeline_stages (repo_id, stage_name, stage_order, trigger_after, provider) \
                 VALUES ('test/repo', 'test-stage', 100, 'review_pass', 'self')",
                [],
            )
            .unwrap();
            conn.execute(
