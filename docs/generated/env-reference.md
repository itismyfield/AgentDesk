...
| Variable | Defined at | Description |
|---|---|---|
| `ANTHROPIC_API_KEY` | `src/server/rate_limit_sync.rs:187` (+1 more) | Priority: 1) OAuth token (Claude Code subscription), 2) ANTHROPIC_API_KEY. |
...
| `OPENAI_API_KEY` | `src/server/rate_limit_sync.rs:102` | --- Codex: ~/.codex/auth.json (CLI subscription), else OPENAI_API_KEY --- |
