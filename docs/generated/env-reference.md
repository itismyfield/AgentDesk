...
| Variable | Defined at | Description |
|---|---|---|
| `ANTHROPIC_API_KEY` | `src/server/rate_limit_sync.rs:194` (+1 more) | Priority: 1) OAuth token (Claude Code subscription), 2) ANTHROPIC_API_KEY. |
...
| `OPENAI_API_KEY` | `src/server/rate_limit_sync.rs:106` | --- Codex rate limits --- Priority: 1) ~/.codex/auth.json (Codex CLI subscription), 2) OPENAI_API_KEY |
