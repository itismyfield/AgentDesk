...
| `AGENTDESK_CONFIG` | `src/config.rs:2932` (+6 more) | The on-disk config path the running server loaded from, resolved with the same precedence as [`load`] (`$AGENTDESK_CONFIG` → runtime root → cwd → home). |
