...
cluster:
  enabled: true
  instance_id: runner-example
  role: runner
  runtime_profile: runner
  api_base_url: http://runner.example.invalid:8791
  intake_routing:
    enabled: true
    mode: enforce
```

예시의 ID와 주소는 운영자가 지정한 노드 ID 및 실제 접근 가능한 주소로 바꾼다.
실행 전용 기능 모드에는 명시적인 `role: runner`와
...
안내 화면을 제공한다. 이 화면은 바이너리에 포함되며 dashboard 자산이나 관리 API를
활성화하지 않는다. 존재하지 않는 경로는 인증 오류 대신 404로 응답한다.
`localhost`와 `127.0.0.1`은 브라우저가 실행 중인 장비를 가리키므로, 다른 장비의
대시보드에는 해당 Hub 주소로 접속한다. 실행 API에는 기존 인증 규칙이 적용된다.
