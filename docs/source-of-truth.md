...
| Routine entrypoints and helper assets | Bundled QuickJS entrypoints: repo `routines/` → `~/.adk/release/routines/`; deterministic Node/Python helpers: repo `routine-helpers/` → `~/.adk/release/routine-helpers/`. Operator-private additions on both release surfaces are seeded before repository assets overlay them. | The routine loader captures executable metadata only from configured `routines.dir` and `routines.additional_dirs`; helper assets are invoked by agent prompts or cron, never through `script_ref`. Deploy tombstones only the four pre-#4902 helper paths in staged `routines/`, so an old-binary layout cannot linger while unrelated operator assets survive. | Keep executable QuickJS routine entrypoints in `routines/`; put deterministic Node/Python support modules in `routine-helpers/`. Do not put `routine-helpers/` in `routines.additional_dirs`. Operator-private assets remain ignored unless explicitly promoted as a bundled asset. | `git ls-files routines routine-helpers`; `find ~/.adk/release/{routines,routine-helpers} -maxdepth 2 -type f | sort` |
