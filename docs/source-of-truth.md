...
| Policy SQL guard and capability manifest spec | [`docs/policy-sql-guard.md`](policy-sql-guard.md) | Runtime enforcement lives in `src/engine/sql_guard.rs`; future per-policy manifests should be repo files beside their policy. | Edit the spec before changing guard scope or adding a policy capability manifest. | `cargo test --features legacy-sqlite-tests --bin agentdesk policies_raw_db_count_stays_within_budget` |
...
The plist is generated, not hand-written; structural changes belong in the generator (`src/cli/init.rs`). Operator overrides go into `~/.adk/release/config/launchd.env`, which the generator and deploy scripts source. Pitfall: editing the plist directly drifts it from the generator and is reverted on the next `agentdesk emit-launchd-plist`. Use the env overlay for everything operator-tunable. The release plist also carries a generated soft `NumberOfFiles` limit; keep that in the generator so deploys preserve the FD budget without lowering a host-tuned hard limit.
