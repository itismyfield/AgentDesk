...
| Migrated routine metadata | Repo routine JS under `routines/migrated-launchd/*.js` plus per-row `checkpoint` overrides | The routine loader captures `metadata.migrated_launchd.entrypoint`, `required_connectors`, and `required_paths`; `POST /api/routines/<id>/resume` validates the merged metadata/checkpoint contract before enabling a migrated routine. | Edit repo routine metadata for stable requirements. Use per-row checkpoint only for operator-specific overrides. | `cargo test migrated_launchd_` |
...
| Memento workspace memory | Memento MCP-managed store on disk (operator-private) or PostgreSQL `local_memory` fallback created by `src/server/routes/memory_api.rs` when Memento is unavailable or `ADK_FORCE_LOCAL_MEMORY=1`. Scope contract: [`docs/memory-scope.md`](memory-scope.md) (#1100). | `permanent` is for user identity, preferences, long-term decisions, and resolved procedures. `workspace`/`session` is for project/turn-local state. File-canonical content (prompts, runtime config, policies, skills, memory tiers listed in this matrix) MUST NOT be mirrored into Memento. When unsure, write `workspace`. | Use the Memento tool surface (`remember`, `amend`, `forget`, `memory_consolidate`) for MCP-backed data. For fallback rows, use `/api/memory/{recall,remember,forget}` or direct SQL against `local_memory`; split audit results by API `source` / `detected_backend` because recall/forget bridges for MCP-backed Memento are not complete. Avoid file-level edits to the MCP store. Promote `workspace` → `permanent` only via explicit `amend` after multi-session validation. | MCP-backed: `mcp__memento__memory_stats` and `mcp__memento__context`; fallback: `psql "$DATABASE_URL" -c "SELECT id, topic, kind, workspace, created_at FROM local_memory ORDER BY created_at DESC LIMIT 20;"` or `curl -sf -X POST http://127.0.0.1:8791/api/memory/recall -H 'Content-Type: application/json' -d '{"limit":20}'` |
...
Memento data is stored under the operator-private MCP server or the PostgreSQL `local_memory` fallback. MCP-backed data is accessed through the `mcp__memento__*` tool surface; `/api/memory/*` is route-level dual-mode only for implemented bridges, and recall/forget must not be treated as full MCP-backed Memento access. The fallback path is owned by `src/server/routes/memory_api.rs`: it creates, recalls, inserts, and deletes rows in `local_memory`, and it is selected when Memento is unavailable or `ADK_FORCE_LOCAL_MEMORY=1` is set.

Audit these as two cases. For MCP-backed memory, use `mcp__memento__memory_stats` and `mcp__memento__context`. For fallback memory, query the table directly with `psql "$DATABASE_URL" -c "SELECT id, topic, kind, workspace, created_at FROM local_memory ORDER BY created_at DESC LIMIT 20;"` or call `curl -sf -X POST http://127.0.0.1:8791/api/memory/recall -H 'Content-Type: application/json' -d '{"limit":20}'` and verify the response reports `source: "local"` while recording `detected_backend`. Because MCP recall/forget bridges are still incomplete, a clean `/api/memory/recall` result only clears the fallback table, not the MCP-backed Memento store.

The permanent-vs-workspace scope contract is owned by [`docs/memory-scope.md`](memory-scope.md) (issue #1100). Permanent scope is reserved for user identity, preferences, long-term decisions, and resolved procedures; everything else is `workspace`. Any content that has a canonical home in another row of this matrix (prompts, runtime config, policy files, skills, memory tiers) MUST NOT be mirrored into Memento. The Memento tool responses are authoritative for what they return — do not mirror Memento content back into repo files either.
