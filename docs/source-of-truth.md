...
| Host-wide Cargo build token | Repo `scripts/build_token.py` + repo-only `scripts/_build_token.sh`. POSIX cooperative authority: `/tmp/adk-build-token.lock`; Win32 authority: the SID-scoped mutex in `scripts/build_token_win32.py`. | Release entrypoints source `_build_token.sh` and hold the host token before Cargo takes its target lock. The POSIX supervisor alone retains the non-inheritable flock FD and reaps the foreground Cargo process; trusted helpers never unlink/recreate the path. An observed stale waiter fails closed, but this single-path theorem does not claim to exclude a same-UID third entrant on a replacement inode. | Change the helper and `tests/test_build_token_5663.py` together. Do not add inline wrappers, held booleans/FDs, alternate production paths, nested/whole-deploy wrapping, explicit Cargo jobs flags, raw Cargo fallbacks, or target-directory bypasses. Canonical skill edits referencing a new helper happen only after that helper is merged to main; the non-Git vault is tracked by this versioned contract plus recorded hashes, never by editing mirrors. | `python3 -m unittest -v tests.test_build_token_5663` and `bash tests/test_build_token_5663.sh` use temporary authorities and dummy commands; they never touch the live token or run Cargo/deploy. |
