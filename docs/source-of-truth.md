...
| Operator routine scripts | Operator-private local `~/.adk/release/workspaces/agentdesk/routines/**/*.js`, staged to `~/.adk/release/routines/` on deploy when present. Git ignores this tree; public repo history must not gain personal routine scripts. | The deploy scripts use `AGENTDESK_ROUTINES_SRC` or the canonical release workspace path, not the invoking worktree's `routines/`, so deploys from isolated worktrees do not overwrite runtime routines with stale local copies. The GitHub release artifact builder excludes routines by default and includes them only with `AGENTDESK_BUILD_INCLUDE_ROUTINES=1`. The routine loader captures script metadata from the configured `routines.dir` plus optional `routines.additional_dirs`; `POST /api/routines/<id>/resume` validates metadata/checkpoint contracts before enabling a migrated routine. If local `routines/` is absent during deploy, the existing release runtime routines are retained. | Edit the operator-private local/runtime routine file. Do not commit routine JS to GitHub; repo changes should be limited to runtime engine code, API contracts, docs, or non-personal examples outside `routines/`. | `git ls-files routines` must be empty; `find ~/.adk/release/workspaces/agentdesk/routines -maxdepth 2 -type f` checks local-only scripts |
