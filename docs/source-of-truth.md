...
| Operator routine scripts | Operator-private local `routines/**/*.js` in the release workspace, staged to `~/.adk/release/routines/` on deploy when present. Git ignores this tree; public repo history must not gain personal routine scripts. | The routine loader captures script metadata from the configured `routines.dir` plus optional `routines.additional_dirs`; `POST /api/routines/<id>/resume` validates metadata/checkpoint contracts before enabling a migrated routine. If local `routines/` is absent during deploy, the existing release runtime routines are retained. | Edit the canonical workspace `routines/**/*.js` (under `AGENTDESK_REPO_DIR`, default `workspaces/agentdesk/routines/`). The `~/.adk/release/routines/` copy is a deploy-staged mirror that the next deploy overwrites with the workspace source, so edits made only to the release mirror are reverted — mirror the workspace source too, on every deploy-participating machine (see the per-vector detail). Do not commit routine JS to GitHub; repo changes should be limited to runtime engine code, API contracts, docs, or non-personal examples outside `routines/`. | `git ls-files routines` must be empty; `find routines -maxdepth 2 -type f` checks local-only scripts |
...
### Operator routine scripts

Migrated launchd routines and their `.js` dispatchers are operator-private and git-ignored. The canonical source is the workspace tree `routines/**/*.js` under `AGENTDESK_REPO_DIR` (default `~/.adk/release/workspaces/agentdesk/routines/`). `scripts/deploy-release.sh` stages routines by first copying `~/.adk/release/routines/` and then **overlaying the workspace `routines/` on top** (`rsync -a`, staging block ~L1300–1315), so `~/.adk/release/routines/` is only a deploy-staged mirror. Pitfall (same shape as the prompt-mirror pitfall above): editing only the release mirror `~/.adk/release/routines/**/*.js` survives until the next deploy and is then silently reverted to the workspace version — this is the cause when an operator routine edit "keeps coming back" after a deploy. Always edit the canonical workspace `routines/` source; update the release mirror too only for immediate effect before the next deploy. In a multi-node cluster mirror BOTH on every deploy-participating machine — the deploy runs from one node (which may be STANDBY per `/api/health/detail cluster_standby`) and its routine rsync overwrites the peers' operator routines, so the ACTIVE node that actually fires the routine gets reverted. Keep heavy per-routine logic in a durable ObsidianVault skill script and let the `.js` stay a thin dispatcher, so a stale mirror can never ship a stale copy of the logic.
