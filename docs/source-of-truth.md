...
| Host-wide Cargo build token | Repo `scripts/build_token.py`; fixed authority path `/tmp/adk-build-token.lock`. | Release entrypoints call the shared `_with_build_token` facade in `scripts/_defaults.sh`; Cargo then takes its own target lock inside the host token. The token file persists and is never unlinked/recreated by callers. | Change the Python helper and `tests/test_build_token_5663.py` together. Do not add inline Python flock wrappers, boolean held flags, alternate token paths, whole-deploy wrapping, or target-directory bypasses. | `python3 -m unittest -v tests.test_build_token_5663` and `bash tests/test_build_token_5663.sh` use temporary token paths and never touch the live token. |
