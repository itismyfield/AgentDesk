...
>
> Last refreshed: 2026-07-26 (#4912 review repair — lifecycle-notifier manual delivery now applies a source-scoped transport policy that suppresses every Discord mention class across both inline and attachment compatibility paths. Ordinary manual and DM sources retain the existing relay policy that permits directed user mentions while suppressing everyone and role mentions. The shared manual message builder centralizes this distinction; the final repair replaces test-only serialization `expect`s with typed `Result` assertions without changing production transport or callsites. No delivery verb, v3 protocol shape, dedup identity, target grammar, or production callsite category changed.)
