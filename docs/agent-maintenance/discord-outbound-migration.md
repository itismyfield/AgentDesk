...
> Last refreshed: 2026-07-11 (#4438 — the cfg(test)-only OFF long-chunk
...
>
> Last refreshed: 2026-06-20 (against #3593 codex HIGH — restores the generation-checked, flag-independent durable-frontier-end reader `delivered_frontier_end_current_generation` in `outbound/delivery_record.rs` and fuses it via `max` with the in-memory committed offset in `committed_floor_for_resend_dedup`. The synthetic-resume / placeholder re-send dedup gate in `tmux_watcher.rs` reads this fused floor so it stays a TRUE superset of the #3520 new-message guard; `effective_committed_offset` is production default-ON as of #4890, while the cfg(test) OFF seam retains the legacy in-memory path. `max` only raises the floor and the reader is current-generation-only per the #1270 guard, stale-gen → 0, so a genuinely-new post-reset answer is never over-suppressed. Earlier under #3520 the watcher-direct fallback gated `has_direct_terminal_response` on the same reader to suppress re-mirroring already-delivered text as a duplicate message at the same prompt anchor. Outbound delivery semantics otherwise unchanged).
>
...
>
> Last refreshed: 2026-07-26 (#4890 — delivery-record shadow writes and durable-frontier authority are production default-ON code paths; the two production environment gates were removed. The per-thread `cfg(test)` seams retain deterministic OFF coverage for legacy in-memory dedup and observe-only backward-write behavior. Durable frontier generation/EOF validation, confirmed-commit ordering, backward-write reset carve-outs, and retry-reset handling are unchanged; health now reports the default `shadow_and_authority` mode without configuration warnings).
>
