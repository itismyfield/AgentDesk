...
> Last refreshed: 2026-07-30 (#5022 — every successful v3 Discord transport records process-lifetime, per-channel monotonic delivery evidence in `outbound/delivery_evidence_store.rs`; mailbox turn-start snapshots its baseline and relay-dead classification consumes only the delta for that turn. This adds no transport verb or producer callsite, so the coverage rows below are unchanged).
>
