...
> Last refreshed: 2026-07-30 (#5022 — relay-dead recovery consumes closed process-local lease evidence: `NotAttempted`, `NotDelivered`, `AttemptedUnconfirmed`, `Delivered`, or `Unknown`. Only exact-key `NotDelivered` can arm the recorded-zero dead-frontier predicate. Active lease attempts are blocked separately; ambiguous release/task abort/lease expiry, degenerate id-0 keys, markerless `NoLease` standby/recovery transport, TUI-direct/external-input turns, and bridge `NoCommitOnFallback` fallback delivery are `Unknown` and fail closed. This is not a claim that every successful v3 transport is observed. Covering those turn classes requires a separate issue that moves them behind one authoritative lease/commit boundary; transport verbs and coverage rows below are unchanged).
>
