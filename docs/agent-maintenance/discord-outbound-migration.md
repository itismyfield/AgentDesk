...
| `decide_policy_with_limits(...) -> DiscordOutboundPolicyDecision` | `outbound/decision.rs:137` | active pure planner | Pure function that turns a v3 message + policy into a delivery plan (split / fallback / dedup). Does not perform I/O. |
