...
> Last refreshed: 2026-07-18 (#4046 S1r-1 — `outbound/turn_output_controller.rs` gains a pure-add anchor-less `OutputPlan::SendFresh { range, reference, record }` verb (body threaded via `TurnOutputCtx::body`), housed in the new child module `turn_output_controller/fresh_send.rs`. It is NOT wired to any live owner path yet (dormant; zero production caller), so the production callsite coverage map is unchanged. NoRange sends refuse duplicates via the existing `recent_delivered_content_matches` (#4081 fingerprint) before POST and record a current-generation content fingerprint after success, plus a body-byte-length pseudo-range lease to serialize concurrent fresh sends; NoRange never records a durable frontier nor claims dedup authority. The range path still records the durable frontier + new message anchor only after a successful lease commit. `outbound/delivery_record.rs` change is a 1-line internal widening; no new durable writer, retry, cleanup, or delivery API shape.)
