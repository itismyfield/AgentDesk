...
- do_not_edit_without_migration_plan (giant-file):
  ...
  - `src/services/discord/router/message_handler/headless_turn.rs` (1533 lines; +64 from #family-profile-probe DM-fresh provider session: `dm_fresh_routine_turn` discriminator routes a fresh DM routine turn through the shared `/goal fresh` machinery (`force_fresh_provider_session = goal_fresh || dm_fresh`) — thorough clear (in-memory + DB + stale id) + DB/live-TUI restore skip + launch fresh flag so the live tmux pane is not reused (codex review P1/R2 — in-memory clear alone is undone by the persisted id, and the live pane is reused via runtime-binding recovery); the `/goal` prompt rewrite stays goal-only so the probe prompt is sent verbatim; so a fresh DM routine turn never resumes the accumulated per-channel session (memento caseId is the only cross-run continuity); -45 from #3591 100턴 세션 리셋(AssistantTurnCap) 제거: reset 판정/clear/DB clear/display 블록 삭제; -2 from #3588 idle 세션 리셋 제거(IdleExpired display arm + `now` 인자 정리);
