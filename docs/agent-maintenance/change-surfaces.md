...
- do_not_edit_without_migration_plan (giant-file):
  ...
  - `src/services/discord/router/message_handler/headless_turn.rs` (1529 lines; +60 from #family-profile-probe DM-fresh provider session: `dm_fresh_routine_turn` discriminator + turn-start `clear_provider_session()` AND skipping the DB/live-TUI provider-session restore for fresh DM routine turns (codex review P1 — the in-memory clear alone is undone by the persisted provider id), so a fresh DM routine turn never resumes the accumulated per-channel session (memento caseId is the only cross-run continuity); -45 from #3591 100턴 세션 리셋(AssistantTurnCap) 제거: reset 판정/clear/DB clear/display 블록 삭제; -2 from #3588 idle 세션 리셋 제거(IdleExpired display arm + `now` 인자 정리);
