...
> Last refreshed: 2026-09-18 (against #5941 Step B DLQ redelivery sweep).

### Audited touches
- 2026-09-18 (#5941 Step B): the DLQ redelivery sweep is a second background singleton beside the intake-delivery sweep. Its `SWEEP_ACTIVE` latch is process-local, so it bounds one sweep per process, not per cluster, and unlike the sibling sweep it is set without a `Drop` guard: the per-tick `catch_unwind` wraps only `sweep_once_with`, so a panic from `interval.tick()` or the http fallback escapes, `spawn_observed` does not respawn, and the latch stays `true` with no reader for the rest of the process lifetime. Cross-node exclusivity does not rest on that latch — the claim is one `UPDATE ... WHERE redelivery_state = 'pending' ... FOR UPDATE SKIP LOCKED`, so concurrent sweeps on any node partition the backlog and each row is handed out once. There is no PG lease and leader-only is not a correctness requirement. A settle CAN return a row to `'pending'`: an unreadable witness or a failed POST is not a verdict about the body, so the row is claimed again rather than retired. What still strands a row is a node dying between claim and POST, which leaves it `'claimed'` with no reclaimer (#6009).
