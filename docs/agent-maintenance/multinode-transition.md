...
- 2026-07-24 — #4508 edit-failure fallback authority: controller and legacy
  watcher owners now re-read the worker-local, current-generation, EOF-bounded
  durable frontier while retaining the same process-local delivery lease. This
  adds no cross-node adoption authority, leader election, PG lease, or routing
  change; unverifiable sidecars remain fail-open for delivery.
