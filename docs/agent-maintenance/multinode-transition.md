...
> Last refreshed: 2026-08-30 (against #5191 writer protocol W0).

### Audited touches
- 2026-08-30 (#5191 W0): `services::writer_protocol` is dormant. Its registry is
  process-local only, and pure control-lock path derivation is not lock
  authority. A future `flock` would remain host-local rather than node/fleet
  authority. Provider-native transcripts and rollouts remain observation-only;
  no production writer or cleanup caller consumes the protocol in W0.
