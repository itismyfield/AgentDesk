...
- **잔여 P1 폐쇄 — `line_integrity` 분모를 대상 구간 자신의 레코드로 교체**(legA/legB r3c P1-1).
  ...
  **대상에 통째로 부과**된다 → 남의 오염이 깨끗한 대상을 FAIL 시킬 수 있다. 이 잔여와, 대상
  레코드가 하나도 없는 파일이 스코프 밖이라는 기존 잔여는 **둘 다 false-red 방향**이며, 승격
  게이트가 틀려도 되는 방향이 그쪽이다. r3 문면이 배제 방향만 선언하고 **포함 방향(fail-open)을
