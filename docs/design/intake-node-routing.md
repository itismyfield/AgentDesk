...
| Worker dies before claim | row stays `pending` past `forward_pre_claim_timeout_secs` (12s) | **Transition 7 is not implemented in the current runtime.** The current guard moves the stale row to `failed_pre_accept` for the existing leader sweep; no local re-target/attempt INSERT occurs yet. Retry exhaustion remains bounded by transition 10 and alerts the operator. |
