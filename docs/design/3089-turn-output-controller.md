...
- **B3 demote memory lease to a cache + enforce the guard** — promote the #3154 observe-only monotonic guard to **enforce** now that a single authority exists (closes #3416). **AC6 met here.**
  ...
  - **Deferred to post-canary (NOT in this default-OFF phase):** deprecating the legacy duplicate markers behind a version bump — the legacy markers remain the fallback until the flag has been validated ON in production; removing them now (flag still default OFF) would be premature.
