...
- **B3 demote memory lease to a cache + enforce the guard** — promote the #3154 observe-only monotonic guard to **enforce** now that a single authority exists (closes #3416). **AC6 met here.**
  - **As-built (B3):** the enforce is `validate_inflight_state_for_save` returning a verdict the plain-overwrite save tails (`save_inflight_state_in_root`, `persist_under_lock`) honor by skipping a same-turn BACKWARD offset write (caller keeps the higher on-disk state). The pure `delivery_record::authority_blocks_backward_inflight_write` helper shares the durable-frontier authority decision; production is default-ON and the `cfg(test)` seam preserves deterministic observe-only coverage. The debug-build `debug_assert` still fires first in debug; enforce is the release-build graceful skip.
  - **As-built visibility (#3746/#4890):** `/api/health` and `/api/health/detail` expose `delivery_record_rollout`, including shadow/authority booleans, rollout `mode`, effective dedup authority, same-turn backward-write enforcement mode, and warning count. Production reports `shadow_and_authority` with zero configuration warnings; the test-only OFF seams retain legacy-path regression coverage.
  - **As-built recovery owner key (#3751):** `InflightTurnState` carries `watcher_owner_channel_id` for same-binary fast reads, while the mixed-binary-safe restart source is the separate `discord_delivery_owner_context` sidecar, written under the delivery channel and pointing to the watcher-owner channel plus tmux generation. Restart recovery resolves the sidecar first, falls back to legacy inflight/channel keys, and refuses shared-owner anchors whose recorded `panel_channel_id` does not match the recovered delivery channel. `logical_channel_id` is not a delivery-record key.
  - **Restart hydration is realized as read-fusion, not write-back:** B2b/B2c route EVERY `confirmed_end_offset` consumer (sink dedup, the 3 watcher dedup reads, the idle-tail clamp) through `effective_committed_offset`, which already fuses `max(durable delivered_frontier, in-memory)` under the #1270 generation guard. That delivers the design's "no 0-reset on restart" at every read; an additional startup write-back into the in-memory atomic is redundant and riskier (would fight the regression-reset path), so it is intentionally not done.
  - **`delivery_lease` reconcile is N/A:** no production path ever wired a live lease writer (`upsert_lease`/`clear_lease` remain `#[allow(dead_code)]`); the durable authority that emerged is the release-surviving `delivered_frontier` alone, so there is no leftover lease to reconcile.
  - **Deferred versioned cleanup:** deprecating the legacy duplicate markers remains a separate binary-version migration. Production binaries now run delivery-record SHADOW and AUTHORITY code-level default-ON; there is no production environment flag that can roll either path OFF, and the `cfg(test)` OFF seams are regression fixtures rather than rollback controls. Operational rollback therefore means deploying a prior binary or shipping a forward fix. Until a coordinated binary cleanup proves mixed-version safety, the legacy markers remain available for older-binary compatibility.
