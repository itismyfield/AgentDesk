...
- **B3 demote memory lease to a cache + enforce the guard** — promote the #3154 observe-only monotonic guard to **enforce** now that a single authority exists (closes #3416). **AC6 met here.**
  - **As-built (B3):** the enforce is `validate_inflight_state_for_save` returning a verdict the plain-overwrite save tails (`save_inflight_state_in_root`, `persist_under_lock`) honor by skipping a same-turn BACKWARD offset write (caller keeps the higher on-disk state). Gated by the SAME `AGENTDESK_DELIVERY_RECORD_AUTHORITY` flag as the B2 read-flip via the pure `delivery_record::authority_blocks_backward_inflight_write` helper, so the single authority + its enforcement cut over atomically; flag OFF (default) → byte-identical observe-only, deploy no-op. The debug-build `debug_assert` still fires first in debug; enforce is the release-build graceful skip.
  - **As-built visibility (#3746):** `/api/health` and `/api/health/detail` expose `delivery_record_rollout`, including shadow/authority booleans, rollout `mode`, effective dedup authority, same-turn backward-write enforcement mode, and warning count. This does not flip authority; it makes the default-OFF / shadow-only / enforcing state operator-visible before any production rollout decision.
