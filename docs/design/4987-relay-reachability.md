...
| # | 검출 항 | 지연 | 메커니즘 |
|---|---|---|---|
| 1 | **`RowlessActiveTurn`** | `UNPAIRED_ACTIVE_TOKEN_GRACE_SECS` (600s, 구현값) | mailbox `has_cancel_token=True` + `active_user_message_id` 존재 + `inflight_state_present=False` 가 bound 이상 지속. **워터마크를 기록할 행이 없다 = 배달 불가능의 구조적 증명**. 턴 승인~행 생성 사이 정상 창을 덮기 위해 즉시가 아닌 bound |
