...
| PG `intake_outbox` (terminal statuses)    | `done` 7 days; `unknown`/`failed_pre_accept`/`failed_post_accept` 30 days | Batched `DELETE` (1000 × 50 per run); currently count-only (`intake_outbox:delete_would`) | `storage.db_retention` | Weekly |
