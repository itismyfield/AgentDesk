...
>
> #6014: `ci-pr.yml`의 `pg_db` filter 중 **소스 경로 부분은 생성물**이다.
> `scripts/pg_test_lane_manifest.txt`의 `[files]`가 `# BEGIN/END generated
> pg_db source paths` 구간으로 렌더링되고, 재생성은
> `python3 scripts/check_pg_test_lane_membership.py --write-pg-db-paths`다.
> 구간 밖의 수작업 글롭·비소스 트리거(`migrations/**`, `justfile`, `Cargo.*`
> 등)와 주석은 보존된다. 기본 게이트가 구간 동기화를 함께 검사하고, 무조건
> 실행되는 `scripts` 잡의 `ci-script-checks.sh`가 재생성 후
> `git diff --exit-code`로 **커밋된 트리**가 생성 결과와 같은지 확인한다.
> filter는 last-match-wins이라 생성 구간보다 **앞선** `!` 패턴은 조용히
> 무력화되지 않고 rc=2로 거부되며, 구간이 없거나 중복·역순이어도 파일을 고치지
> 않고 rc=2다. 이로써 `[rule3]` debt는 21 → 0이고 baseline의 해당 섹션은 비었다.
> 이 변경은 PR에서 PG 레인이 **선택되는 빈도**만 바꾼다 — 선택된 잡은 이전에도
> 마커가 붙은 테스트를 전부 실행했으므로 잡 실행 시간에 대한 주장이 아니다.
>
