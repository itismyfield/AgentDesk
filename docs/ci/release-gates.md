...
| --- | --- | --- | --- | --- |
| **Full tests** | `full_non_pg` | `check_fast` (compile/policy + 11 targeted Rust invocations) + `test_fast` | `full_macos` + `full_windows` | main/nightly always run broad non-PG tests; the path-filtered PR lane runs the reviewed targeted subset. |
