...
echo "=== Reachability canonical Rust<->Python equivalence gate (#5071 T4-B2) ==="
...
# side red), checks that every curated lane filter still selects at least one
# real test, and lints the T4-B2 no-judgment-authority invariant: no consumer
# outside the tree, no warn/fail bound inside it. The RUST half of the mutation
# runner needs a compiler and runs under `--with-rust` (see `just
# reachability-mutation-runner`); what CI holds instead is the lane, plus the
# unittest below, which fails if a declared Rust mutation stops anchoring on
# real source and would therefore be silently skipped.
