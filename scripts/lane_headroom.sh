...
capture_memory_pressure() {
  ...
  MEMORY_PRESSURE_OUT="$(memory_pressure 2>/dev/null)"
}
...
probe_swapouts_counter() {
  ...
  # is the fallback. Re-invoked per sample — never reuse a cached reading, or
  # the delta is structurally 0.
  local v=""
  if command -v vm_stat >/dev/null 2>&1; then
    v="$(vm_stat 2>/dev/null | awk -F: '/^Swapouts/ { gsub(/[^0-9]/, "", $2); if ($2 != "") { print $2; exit } }')"
  fi
  if [ -z "$v" ] && command -v memory_pressure >/dev/null 2>&1; then
    v="$(memory_pressure 2>/dev/null | awk -F: '/^Swapouts/ { gsub(/[^0-9]/, "", $2); if ($2 != "") { print $2; exit } }')"
  fi
  printf '%s' "$v"
}
...
probe_disk_free_gb() {
  ...
  df -Pk "$path" 2>/dev/null | awk 'NR == 2 && $4 ~ /^[0-9]+$/ { printf "%.1f", $4 / 1048576 }'
}
...
if ! is_num "$cpu_idle"; then
  ...
  details+=("$(printf '  %-13s %s' 'cpu-idle' "unreadable (top -l 1 -n 0 gave no 'CPU usage' line) — need >= ${MIN_CPU_IDLE_PCT}%")")
...
if ! is_num "$mem_free"; then
  ...
  details+=("$(printf '  %-13s %s' 'mem-free' "unreadable (memory_pressure gave no 'System-wide memory free percentage') — need >= ${MIN_MEM_FREE_PCT}%")")
...
if ! is_num "$mem_level"; then
  ...
  details+=("$(printf '  %-13s %s' 'mem-pressure' "unreadable (kern.memorystatus_vm_pressure_level) — need <= ${MAX_MEM_PRESSURE_LEVEL}")")
...
if ! is_num "$swap_before" || ! is_num "$swap_after"; then
  ...
  details+=("$(printf '  %-13s %s' 'swapout-rate' "unreadable (no swapouts counter from vm_stat or memory_pressure) — need <= ${MAX_SWAPOUT_RATE} pages/s")")
else
...
if ! is_num "$disk_free"; then
  ...
  details+=("$(printf '  %-13s %s' 'disk-free' "unreadable (df -Pk ${DISK_PATH}) — need >= ${MIN_DISK_FREE_GB}GB")")
