...
def native_chain(path, before, request, session):
    ...
    inputs = [i for i, (_, row) in enumerate(rows)
              if row.get("type") == "user" and request["prompt"].strip() in content(row)]
    if len(inputs) > 1:
    ...
    for loc, row in rows[index + 1:]:
        if row.get("type") == "user" and content(row):
            break  # tool_result-only user entries are not new input turns.
...
class Evidence:
    def __init__(self, driver, args, channel, record):
        ...
        sample_start = {**self.log_cursor, "offset": max(0, self.log_cursor["offset"] - 65536)}
    ...
    def idle(self):
        ...
        state = self.watcher()
        if state.get("unread_bytes") is None:
            raise ValueError("E36 unread_bytes unmeasured")
        if state["unread_bytes"] != 0 or state.get("has_pending_queue") is not False:
    ...
    def queue(self, request, prior):
        matches = []
        deadline = min(prior["deadline"], self.d.time.monotonic() + 20)
        while self.d.time.monotonic() < deadline and not matches:
            ...
            if str(state.get("mailbox_active_user_msg_id")) != prior["inbound_message_id"]:
                raise ValueError("E36 QA activity at QB commit no longer observable; exact commit retained if available")
            if not matches:
                self.d.time.sleep(0.1)
        if not matches:
...
def run(evidence, scenario, client, window, record, run_id, after_id):
    ...
    def send(step):
        ...
        request.update(turn_identity=identity, inbound_message_id=identity["user_msg_id"],
                       deadline=request["acknowledged_at"] + 240)
    ...
    def complete(request, excluded=()):
        ...
        ingest(observed)
        if not found:
            raise assertions.AssertionError("E36 response missing within 240s")
        ...
        try:
            ...
            if request["request_key"] == "QA" and len(completion_candidates(sub)) > 1:
                completion_id(sub, request, ambiguous_error=d.HarnessEvidenceError)
            raise
        request["completion_message_id"] = completion_id(sub, request,
            ambiguous_error=d.HarnessEvidenceError if request["request_key"] == "QA" else assertions.AssertionError)
    ...
    for request in requests:
        ...
        for update in window.message_updates:
            for field in ("before", "after"):
                ...
                if request["body_marker"] not in text:
                    continue
                mid = str(update["id"])
                if mid not in observed:
                    raise ValueError("E36 edited publication author metadata unavailable")
                body = assertions.relay_body({**observed[mid], "content": text})
                if body is not None and request["body_marker"] in body and (
                        mid not in request["response_ids"] or body.count(request["body_marker"]) != 1):
        ...
        for view in (window.raw_messages, rows):
            bodies = {str(row["id"]): body for row in view
                      if (body := assertions.relay_body(row)) is not None and request["body_marker"] in body}
            if (sum(body.count(request["body_marker"]) for body in bodies.values()) != 1
        ...
        if completion_id(request_window(window, request, rows, excluded), request,
                         ambiguous_error=ValueError if request["request_key"] == "QA" else assertions.AssertionError) != request["completion_message_id"]:
