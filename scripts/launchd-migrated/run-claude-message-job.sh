...
for f in "$CHUNK_DIR"/*.txt; do
  ...
  SEND_RESULT="$(curl -s -X POST "http://127.0.0.1:${ADK_PORT}/api/discord/send" \
    ...
    -d "{\"target\":\"channel:${CHANNEL_ID}\",\"content\":${JSON_CONTENT},\"source\":\"${ADK_SEND_SOURCE:-routine-runtime}\",\"bot\":\"${ADK_BOT:-notify}\",\"record_transcript\":true,\"transcript_source_label\":\"${SOURCE}\"}")"
