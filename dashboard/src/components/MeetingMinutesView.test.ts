...
function meeting(overrides: Partial<RoundTableMeeting> = {}): RoundTableMeeting {
  return {
    id: "meeting-1",
    channel_id: "meeting",
...
describe("submitMeetingStartRequest", () => {
  it("shows accepted notification immediately and upgrades the same notification on success", async () => {
    const notifications: Array<{ kind: "notify" | "update"; id: string; message: string; type: string | undefined }> = [];
    const notify = vi.fn((message: string, type?: "info" | "success" | "warning" | "error") => {
      const id = `n-${notifications.length + 1}`;
      notifications.push({ kind: "notify", id, message, type });
      return id;
    });
    const updateNotification = vi.fn((id: string, message: string, type?: "info" | "success" | "warning" | "error") => {
    ...
    const result = await submitMeetingStartRequest({
      ...
      startMeeting: vi.fn(async () => ({ ok: true, message: "회의가 큐에 등록되었습니다" })),
      notify,
      updateNotification,
      t,
    });
    ...
    expect(updateNotification).toHaveBeenCalledWith("n-1", "회의가 큐에 등록되었습니다", "success");
  });
  ...
  it("reuses the same notification lifecycle on failure without emitting a separate success toast", async () => {
    ...
    expect(updateNotification).toHaveBeenCalledWith("n-1", "start failed", "error");
  });
});
...
describe("openMeetingDetailWithFallback", () => {
  it("requests the clicked meeting id and falls back to the cached meeting on fetch failure", async () => {
    const fallbackMeeting = meeting({ id: "meeting-clicked", summary: "fallback" });
    const fetchMeeting = vi.fn(async (_id: string) => {
      throw new Error("boom");
    });
    const logError = vi.fn();

    const result = await openMeetingDetailWithFallback(fallbackMeeting, fetchMeeting, logError);
